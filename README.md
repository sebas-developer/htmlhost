...
- **Multi-key accounts with hierarchy.** One `setup()` creates a root admin. Create scoped sub-keys (`--scope user`, `--scope team`, `--scope admin`) — each gets its own independent account with private pastes. Admin oversees team/user descendants; admins are equal rank (can't see each other's private pastes).
